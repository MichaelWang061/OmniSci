...
def Search(request):
    if request.method == "GET":
        ...
        popular_projects = ProjectInfo.objects.all().order_by('weight').reverse()[:10]

        return render(request, "search/search.html", {"object_list": results, "query": query, "popular_projects": popular_projects})
    else:
